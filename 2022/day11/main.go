...
func parse(old int, lhs string) int {
	switch lhs {
	case "old":
		return old
	default:
...
func (m *Monkey) inspect() []int {
	fs := strings.Fields(m.Operation)
	lhs, op, rhs := fs[2], fs[3], fs[4]

	return lo.Map(m.Items, func(item, i int) int {
		l, r := parse(item, lhs), parse(item, rhs)
		switch op {
		case "+":
			item = l + r
		case "-":
		...
		// After inspection, worry level is divided by 3 and rounded down
		item /= 3
